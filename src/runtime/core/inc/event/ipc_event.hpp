...
public:
    ...
    void IpcEventCountAdd() { totalTaskCnt_++; }
    void IpcEventCountSub() { totalTaskCnt_--; }
    Device* Device_() const override { return device_; }
    Context* Context_() const override { return context_; }
    uint64_t GetEventFlag() const override { return eventFlag_; }
    void SetIsNeedDestroy(bool flag) override { isNeedDestroy_.Set(flag); }
    IpcHandleVa* GetIpcHandleVa() { return ipcHandleVa_; }
    uint8_t* GetCurrentHostMem() const { return RtPtrToPtr<uint8_t*>(currentHostMem_); }
    void IpcVaLockInit() { ipcHandleVa_->lockStatus = static_cast<uint64_t>(LockStatus::LOCK_RELEASED); }
    void IpcVaLock()
    {
    ...
    {
        while (!CompareAndExchange(
            &ipcHandleVa_->lockStatus, static_cast<uint64_t>(LockStatus::LOCK_OCCUPIED),
            static_cast<uint64_t>(LockStatus::LOCK_RELEASED)))
            ;
    }

    void SetIpcFinished(void) { eventStatus_ = RECORDED; }

    bool IsIpcFinished() const { return (eventStatus_ == RECORDED) ? true : false; }
