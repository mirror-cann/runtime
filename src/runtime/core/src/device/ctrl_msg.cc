...
rtError_t CtrlMsgStreamRecycleInit(TaskInfo* taskInfo, const RtCtrlMsgParam& param)
{
    ...
    (void)MaintenanceTaskInit(
        taskInfo, MT_STREAM_RECYCLE_TASK, static_cast<uint32_t>(taskParam.streamId), taskParam.isForceRecycle);
...
rtError_t CtrlMsgModelTaskInit(TaskInfo* const taskInfo, const RtCtrlMsgParam& param)
{
    ...
    (void)ModelMaintainceTaskInit(
        taskInfo, taskParam.mType, taskParam.modelPtr, taskParam.opStreamPtr, taskParam.modelStreamType,
        taskParam.firstTaskIndex);
    return RT_ERROR_NONE;
}
...
rtError_t CtrlMsgAicpuModelInit(TaskInfo* taskInfo, const RtCtrlMsgParam& param)
{
    ...
    (void)ModelToAicpuTaskInit(
        taskInfo, taskParam.modelIndex, taskParam.controlType, taskParam.exeFlag, taskParam.modelPtr);
    return RT_ERROR_NONE;
}
