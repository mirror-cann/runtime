...
public:
    ...
    rtError_t Load(const rtArgsEx_t* const argsInfo, Stream* const stm, ArgLoaderResult* const result) override;
    rtError_t PureLoad(const uint32_t size, const void* const args, ArgLoaderResult* const result) override;
    rtError_t LoadForMix(
        const rtArgsEx_t* const argsInfo, Stream* const stm, ArgLoaderResult* const result, bool& mixOpt) override;
    ...
    H2DCopyMgr* GetPcieBarAllocator(void) { return argPcieBarAllocator_; }

    H2DCopyMgr* GetArgsAllocator(void) { return argAllocator_; }

    H2DCopyMgr* GetSuperArgsAllocator(void) { return superArgAllocator_; }

    H2DCopyMgr* GetMaxArgsAllocator(void) { return maxArgAllocator_; }

    H2DCopyMgr* GetRandomAllocator(void) { return randomAllocator_; }
...
private:
    ...
    rtError_t LoadInputOutputArgs(
        const Stream* const stm, void*& kerArgs, H2DCopyMgr* const umaArgAllocator, bool& copyArgs, const uint32_t size,
        const void* const args, const rtArgsEx_t* const argsInfo) const;
    rtError_t LoadInputOutputArgsForMix(
        const Stream* const stm, void* kerArgs, H2DCopyMgr* const umaArgAllocator, bool& copyArgs, const uint32_t size,
        const void* const args, const rtArgsEx_t* const argsInfo) const;
