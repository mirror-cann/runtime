...
void ConstructSqeForStarsCommonTask(TaskInfo* taskInfo, rtStarsSqe_t* const command)
{
    ...
    const errno_t error = memcpy_s(
        command, sizeof(rtStarsSqe_t), &starsCommTask->commonStarsSqe.commonSqe,
        sizeof(starsCommTask->commonStarsSqe.commonSqe));
    if (error != EOK) {
        command->commonSqe.sqeHeader.type = RT_STARS_SQE_TYPE_INVALID;
        RT_LOG_INNER_MSG(
            RT_LOG_ERROR,
            "Failed to call memcpy_s to copy starsCommTask->commonStarsSqe.commonSqe,"
            " src=%p, dest=%p, dest_max=%zu, count=%zu, retCode=%#x.",
            &starsCommTask->commonStarsSqe.commonSqe, command, sizeof(rtStarsSqe_t),
            sizeof(starsCommTask->commonStarsSqe.commonSqe), error);
    }
...
static void ConstructWriteValueSqePtr(TaskInfo* const taskInfo, rtStarsSqe_t* const command)
{
    ...
    RT_LOG(
        RT_LOG_DEBUG, "WriteValueTaskPtr streamId=%d, taskId=%hu, addr=%#." PRIx64, taskInfo->stream->Id_(),
...
void ConstructSqeForWriteValueTask(TaskInfo* taskInfo, rtStarsSqe_t* const command)
{
    ...
    RT_LOG(
        RT_LOG_DEBUG, "WriteValueTask streamId=%d, taskId=%hu, addr=%#." PRIx64, taskInfo->stream->Id_(), taskInfo->id,
        writeValTsk->addr);
}
#endif
