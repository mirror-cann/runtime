    ...
    void* const overflowAddr)
{
    ...
    if (reduceAsyncV2TaskInfo->guardMemVec == nullptr) {
        ...
        COND_RETURN_AND_MSG_OUTER(
            reduceAsyncV2TaskInfo->guardMemVec == nullptr, RT_ERROR_MEMORY_ALLOCATION, ErrorCode::EE1013,
...
void ToCommandBodyForReduceAsyncV2Task(TaskInfo* const taskInfo, rtCommand_t* const command)
{
    ...
    if (reduceAsyncV2TaskInfo->overflowAddrOffset != INVALID_CONTEXT_OVERFLOW_OFFSET) {
        ...
        const rtError_t error = driver->MemAddressTranslate(
            static_cast<int32_t>(stream->Device_()->Id_()), RtPtrToValue<void*>(reduceAsyncV2TaskInfo->overflowAddr),
            &offset);
        COND_RETURN_VOID(
            error != RT_ERROR_NONE, "Failed to translate address to physic for reduce async v2 address, retCode=%#x.",
            error);
        RT_LOG(
            RT_LOG_INFO, "ReduceAsyncV2Task offset=%#" PRIx64 ", dir=%u, size_=%" PRIu64, offset,
    ...
    command->u.reduceAsyncV2Task.srcBaseAddr = RtPtrToValue<void*>(reduceAsyncV2TaskInfo->src);
    command->u.reduceAsyncV2Task.dstBaseAddr = RtPtrToValue<void*>(reduceAsyncV2TaskInfo->destPtr);
