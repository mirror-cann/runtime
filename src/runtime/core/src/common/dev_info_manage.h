...
struct hash<rtChipType_t> {
    size_t operator()(const rtChipType_t t) const { return static_cast<size_t>(t); }
};

template <>
struct hash<cce::runtime::RtOptionalFeatureType> {
    size_t operator()(const cce::runtime::RtOptionalFeatureType t) const { return static_cast<size_t>(t); }
};
} // namespace std
...
#define GET_ALL_DEV_PROPERTIES(properties) cce::runtime::DevInfoManage::Instance().GetAllDevProperties(properties)
