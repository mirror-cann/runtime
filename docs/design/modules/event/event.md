...
graph TB
    subgraph API["接口层"]
        aclrtCreateEvent["aclrtCreateEventExWithFlag"]
    ...
    subgraph EventSub["Event 派生层"]
        EventSubClass["Event 子类<br/>CntNotifyId 模式<br/>两态状态机"]
        IpcEvent["IpcEvent<br/>P2P 共享内存同步"]
    end

    subgraph Entry["Record/Wait 入口层"]
    ...
    subgraph Task["任务层"]
        RecordTask["EventRecordTask"]
        WaitTask["EventWaitTask"]
        MemWriteValue["MemWriteValue<br/>IPC Record"]
        MemWaitValue["MemWaitValue<br/>IPC Wait"]
    end

    subgraph StreamLayer["流层"]
        StreamNode["Stream<br/>任务队列管理"]
    end

    aclrtCreateEvent --> Event
    aclrtRecordEvent --> EvtRecord --> EventSubClass
    aclrtStreamWaitEvent --> EvtWait --> EventSubClass
    aclrtIpcGetEventHandle --> IpcEvent --> IpcRecord --> MemWriteValue
    IpcEvent --> IpcWait --> MemWaitValue
    Event --> EventSubClass
    Event --> IpcEvent
    EventSubClass --> StreamNode
    RecordTask --> StreamNode
    WaitTask --> StreamNode
...
sequenceDiagram
    ...
    participant EvtEntry as EvtRecord/EvtWait
    participant EventSub as Event 子类
    participant Stream as Stream
    participant HW as 硬件

    App->>API: aclrtRecordEvent(event, stream)
    API->>EvtEntry: EvtRecord(event, stream)
    EvtEntry->>Stream: CheckTaskCanSend + StreamLock
    EvtEntry->>EventSub: AllocEventIdResource(CntNotifyId)
    EvtEntry->>Stream: AllocTaskInfo + EventRecordTaskInit
    EvtEntry->>HW: SendTask
    EvtEntry->>Stream: StreamUnLock + SubmitTaskPostProc

    App->>API: aclrtStreamWaitEvent(stream, event)
    API->>EvtEntry: EvtWait(event, stream)
    EvtEntry->>EventSub: WaitSendCheck
    EventSub-->>EvtEntry: 已完成，无需下任务
    EvtEntry->>Stream: AllocTaskInfo + EventWaitTaskInit
    EvtEntry->>HW: SendTask
    EvtEntry->>Stream: StreamUnLock + SubmitTaskPostProc
```
