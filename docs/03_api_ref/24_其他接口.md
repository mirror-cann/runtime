...
- [`aclError aclrtGetVersion(int32_t *majorVersion, int32_t *minorVersion, int32_t *patchVersion)`](#aclrtGetVersion（废弃）)：查询接口版本号，acl接口版本号命名采用：A.B.C模式，其中，A表示有不兼容修改，B表示新增接口，C表示bug修复。（废弃接口，请使用aclSysGetVersion接口）
