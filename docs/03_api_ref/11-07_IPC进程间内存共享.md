...
| --- | :---: | --- |
| key | 输入 | 共享内存key<br>必须先调用[aclrtIpcMemGetExportKey](#aclrtIpcMemGetExportKey)接口获取共享内存key，再作为入参传入。 |
| type | 输入 | 内存映射类型。类型定义请参见[aclrtIpcMemAttrType](25-02_Enumerations.md#aclrtIpcMemAttrType)。<br>当前支持配置为ACL_RT_IPC_MEM_ATTR_ACCESS_LINK，用于在跨片访问时指定访问链路类型。不同产品支持的链路类型不同：<br>  - Atlas A3系列产品支持SIO（serial input/output）通道和HCCS（Huawei Cache Coherence System）通道<br>  - Ascend 950PR/950DT产品支持UB单端口路径和UB多端口路径 |
| attr | 输入 | 属性。<br>不同产品的attr取值范围如下：<br><br>**Atlas A3系列产品（训练/推理）**：<br>  - ACL_RT_IPC_MEM_ATTR_ACCESS_LINK_SIO（值为0）：SIO通道<br>  - ACL_RT_IPC_MEM_ATTR_ACCESS_LINK_HCCS（值为1）：HCCS通道<br><br>**Ascend 950PR/Ascend 950DT产品**：<br>  - ACL_RT_IPC_MEM_ATTR_ACCESS_LINK_UB_ONE_PORT_PATH（值为2）：UB单端口路径<br>  - ACL_RT_IPC_MEM_ATTR_ACCESS_LINK_UB_MULTI_PORT_PATH（值为3）：UB多端口路径<br><br><br>宏的定义如下：<br>#define ACL_RT_IPC_MEM_ATTR_ACCESS_LINK_SIO 0<br>#define ACL_RT_IPC_MEM_ATTR_ACCESS_LINK_HCCS 1<br>#define ACL_RT_IPC_MEM_ATTR_ACCESS_LINK_UB_ONE_PORT_PATH 2<br>#define ACL_RT_IPC_MEM_ATTR_ACCESS_LINK_UB_MULTI_PORT_PATH 3 |
...
不同产品支持的attr取值范围不同：
 - Atlas A3系列产品（训练/推理）仅支持attr值为0（SIO）或1（HCCS），若传入其他值将返回[ACL_ERROR_RT_FEATURE_NOT_SUPPORT](25-01_aclError.md)错误码。
 - Ascend 950PR/950DT产品仅支持attr值为2（UB_ONE_PORT_PATH）或3（UB_MULTI_PORT_PATH），若传入其他值将返回[ACL_ERROR_RT_ACCESS_PATH_NOT_SUPPORT](25-01_aclError.md)错误码。
