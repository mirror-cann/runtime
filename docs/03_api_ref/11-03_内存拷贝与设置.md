...
- [`aclError aclrtMemcpyAsyncWithCondition(void *dst, size_t destMax, const void *src, size_t count, aclrtMemcpyKind kind, aclrtStream stream)`](#aclrtMemcpyAsyncWithCondition（废弃）)：实现内存复制。（废弃接口，请使用aclrtMemcpyAsync接口）
...
<a id="aclrtMemcpyAsyncWithCondition（废弃）"></a>
