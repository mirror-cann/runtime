...
TEST_F(ApiAbnormalTest, rtsSetMemcpyDescTest)
{
    ...
    EXPECT_NE(error, RT_ERROR_NONE);
    Api* api = Api::Instance();
    MOCKER_CPP_VIRTUAL(api, &Api::SetMemcpyDesc).stubs().will(returnValue(RT_ERROR_NONE));
    error = rtsSetMemcpyDesc(desc, RT_MEMCPY_KIND_INNER_DEVICE_TO_DEVICE, srcPtr, dstPtr, count, nullptr);
    EXPECT_NE(error, RT_ERROR_NONE);
}
...
TEST_F(ApiAbnormalTest, RtsMemcpyAsyncCountZeroTest)
{
    ...
    MOCKER_CPP_VIRTUAL(impl, &ApiImpl::PtrGetAttributes).expects(exactly(2)).will(returnValue(RT_ERROR_NONE));
    MOCKER_CPP_VIRTUAL(api, &ApiErrorDecorator::MemcpyAsync).expects(never());

    rtError_t error =
        api.RtsMemcpyAsync(dstPtr, sizeof(dstPtr), srcPtr, count, RT_MEMCPY_KIND_DEVICE_TO_DEVICE, nullptr, nullptr);
    EXPECT_EQ(error, RT_ERROR_NONE);
}
...
TEST_F(ApiAbnormalTest, RtsMemcpyTest)
{
    ...
    uint32_t dstPtr[64];
    uint64_t count = 64;
    MOCKER_CPP_VIRTUAL(api, &ApiErrorDecorator::MemCopySync).stubs().will(returnValue(RT_ERROR_NONE));
    MOCKER_CPP_VIRTUAL(impl, &ApiImpl::PointerGetAttributes).stubs().will(returnValue(RT_ERROR_NONE));
...
TEST_F(ApiAbnormalTest, RtsMemcpyCountZeroTest)
{
    ...
    MOCKER_CPP_VIRTUAL(impl, &ApiImpl::PtrGetAttributes).expects(exactly(2)).will(returnValue(RT_ERROR_NONE));
    MOCKER_CPP_VIRTUAL(api, &ApiErrorDecorator::MemCopySync).expects(never());

    rtError_t error = api.RtsMemcpy(dstPtr, sizeof(dstPtr), srcPtr, count, RT_MEMCPY_KIND_DEVICE_TO_DEVICE, nullptr);
    EXPECT_EQ(error, RT_ERROR_NONE);
}
