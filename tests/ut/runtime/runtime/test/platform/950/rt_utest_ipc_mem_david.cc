...
TEST_F(IpcMemDavidTest, IpcMemNameVaMap_set_attr_and_import_simulation)
{
    ...
    MOCKER_CPP_VIRTUAL(driver, &Driver::CheckIpcMapRoute)
            .stubs()
            .will(returnValue(RT_ERROR_NONE));
    ...
    for (int round = 0; round < 3; round++) {
        ...
        EXPECT_EQ(it->second.vaInfo[round].first, expectedAttrs[round]);
        ipcMemNameLock.Unlock();
    }
    ...
    EXPECT_EQ(finalIt->second.vaInfo[0].first, expectedAttrs[0]);
    EXPECT_EQ(finalIt->second.vaInfo[1].first, expectedAttrs[1]);
...
TEST_F(IpcMemDavidTest, IpcMemNameVaMap_import_without_set_attr_default_zero)
{
    ...
    EXPECT_EQ(finalIt->second.vaInfo.size(), 5);
    for (size_t i = 0; i < finalIt->second.vaInfo.size(); i++) {
        EXPECT_EQ(finalIt->second.vaInfo[i].first, expectedDefaultAttr) << "vaInfo[" << i << "] attr should be default 0";
    }
    ipcMemNameLock.Unlock();
...
TEST_F(IpcMemDavidTest, IpcMemNameVaMap_close_ipc_entries_sequentially)
{
    ...
    ipcMemNameLock.Lock();
    EXPECT_EQ(ipcMemNameVaMap.find("ipc_close_test_1"), ipcMemNameVaMap.end());
    EXPECT_EQ(ipcMemNameVaMap.size(), 3);
    ipcMemNameLock.Unlock();
    ...
    ipcMemNameLock.Lock();
    EXPECT_EQ(ipcMemNameVaMap.find("ipc_close_test_2"), ipcMemNameVaMap.end());
    EXPECT_EQ(ipcMemNameVaMap.size(), 2);
    ipcMemNameLock.Unlock();
    ...
    ipcMemNameLock.Lock();
    EXPECT_EQ(ipcMemNameVaMap.find("ipc_close_test_3"), ipcMemNameVaMap.end());
    EXPECT_EQ(ipcMemNameVaMap.size(), 1);
    ipcMemNameLock.Unlock();
