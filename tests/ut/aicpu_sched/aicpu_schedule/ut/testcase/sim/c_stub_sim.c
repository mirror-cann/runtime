...
drvError_t halMemInitSvmDevice(int hostpid, unsigned int vfid, unsigned int dev_id) { return DRV_ERROR_NONE; }
...
{
    (void)session;
    if (dumpChunk == NULL || dumpChunk->fileName == NULL) {
        return IDE_DAEMON_NONE_ERROR;
    }
    // 创建父目录（dump 文件路径可能包含多级目录）
    char dirPath[4096];
    int32_t ret = snprintf_s(dirPath, sizeof(dirPath), sizeof(dirPath) - 1, "%s", dumpChunk->fileName);
    if (ret < 0) {
        return IDE_DAEMON_NONE_ERROR;
    }
    for (char* p = dirPath + 1; *p != '\0'; p++) {
        if (*p == '/') {
            *p = '\0';
    ...
    FILE* fp = fopen(dumpChunk->fileName, "ab");
    if (fp == NULL) {
        return IDE_DAEMON_NONE_ERROR;
    }
    if (dumpChunk->dataBuf != NULL && dumpChunk->bufLen > 0U) {
        (void)fwrite(dumpChunk->dataBuf, 1, dumpChunk->bufLen, fp);
    }
    (void)fclose(fp);
    return IDE_DAEMON_NONE_ERROR;
}
...
/*
 * esched 标准接口由通用仿真平台以强符号接管，此处刻意移除以避免符号竞争：
 *   halEschedWaitEvent / halEschedGetEvent / halEschedAckEvent / halEschedSubmitEvent /
 *   halEschedSubmitEventBatch / halEschedAttachDevice / halEschedDettachDevice /
 *   halEschedCreateGrp / halEschedSubscribeEvent / halEschedSetPidPriority / halEschedSetEventPriority
 */
...
drvError_t halQueueGetMaxNum(unsigned int* maxQueNum) { return DRV_ERROR_NONE; }
...
int halMbufCopy(Mbuf* mbuf, Mbuf** newMbuf) { return 0; }
...
int halGrpCreate(const char* name, GroupCfg* cfg) { return DRV_ERROR_NONE; }
...
drvError_t halEventProc(unsigned int devId, struct event_info* event) { return DRV_ERROR_NONE; }
...
uint32_t aicpuGetDeviceId(uint32_t deviceId) { return DRV_ERROR_NONE; }
...
hdcError_t halHdcGetSessionAttr(HDC_SESSION session, int attr, int* value) { return DRV_ERROR_NONE; }
