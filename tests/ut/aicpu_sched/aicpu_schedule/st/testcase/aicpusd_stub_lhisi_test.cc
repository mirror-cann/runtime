...
protected:
    static void SetUpTestCase() { std::cout << "AICPUScheduleStubLhisiTEST SetUpTestCase" << std::endl; }

    static void TearDownTestCase() { std::cout << "AICPUScheduleStubLhisiTEST TearDownTestCase" << std::endl; }

    virtual void SetUp() { std::cout << "AICPUScheduleStubLhisiTEST SetUP" << std::endl; }
...
{
    aicpu::NotifyFunc notifyFunc = [](void* param, const uint32_t paramLen) { return; };
...
TEST_F(AICPUScheduleStubLhisiTEST, AsyncEventManagerNotifyWait)
{
    ...
    aicpu::AsyncEventManager::GetInstance().NotifyWait(
        reinterpret_cast<void*>(&notifyInfo), sizeof(aicpu::AsyncEventInfo));
    EXPECT_EQ(notifyInfo.retCode, 0);
}

TEST_F(AICPUScheduleStubLhisiTEST, AsyncEventManagerRegEventCb)
{
    auto ret = aicpu::AsyncEventManager::GetInstance().RegEventCb(1, 1, [](void* param) { return; });
    EXPECT_TRUE(ret);
}
...
{
    auto ret = aicpu::AsyncEventManager::GetInstance().RegOpEventCb(1, 1, [](void* param) { return; });
    EXPECT_TRUE(ret);
