...
uint32_t getMbufInMbufListOffSet(uint32_t index) { return mbufSize * index; }
...
// drvError_t halEschedWaitEvent(unsigned int chip_id, unsigned int  thread_index, unsigned int timeoutï¼Œstruct
// event_info *event)
...
TEST_F(AICPUScheduleTEST, ProcessInputDump_invalid_threadId)
{
    ...
    if (opInput != nullptr) {
        ...
        task.UpdateDumpData();
        EXPECT_EQ(
            task.ProcessInputDump(task.baseDumpData_, "", IdeDumpStart(nullptr)), AICPU_SCHEDULE_ERROR_DUMP_FAILED);
    }
}
...
TEST_F(AICPUScheduleTEST, ProcessOutputDump_invalid_threadId)
{
    ...
    if (opOutput != nullptr) {
        ...
        task.UpdateDumpData();
        EXPECT_EQ(
            task.ProcessOutputDump(task.baseDumpData_, "", IdeDumpStart(nullptr)), AICPU_SCHEDULE_ERROR_DUMP_FAILED);
    }
}
...
TEST_F(AICPUScheduleTEST, ExecuteTsKernelTaskLongKernelName)
{
    ...
    kernelInfo.kernelType = 0;
    char* kernelname =
        "TestKernelNameExceptionAbnormalLongOverFlowTestKernelNameExceptionAbnormalLongOverFlowTestKernelNameExceptionA"
        "bnormalLongOverFlowTestKernelNameExceptionAbnormalLongOverFlowTestKernelNameExceptionAbnormalLongOverFlow";
    kernelInfo.kernelBase.cceKernel.kernelName = reinterpret_cast<uintptr_t>(kernelname);
    AicpuEventProcess& aep = AicpuEventProcess::GetInstance();
    EXPECT_EQ(aep.ExecuteTsKernelTask(kernelInfo, 0, 0, 0, 0, 0), AICPU_SCHEDULE_ERROR_NOT_FOUND_LOGICAL_TASK);
}
...
TEST_F(AICPUScheduleTEST, MainTestErr)
{
    ...
    char* argv11[] = {processName, paramDeviceIdOk, paramPidOk,       paramPidSignOk,
                      paramModeOk, paramLogLevelOk, paramGrpNameErr2, paramGrpNumErr2};
    int32_t argc11 = 8;
    ret = ComputeProcessMain(argc11, argv11);
    EXPECT_EQ(ret, -1);
    char* argv12[] = {processName, paramDeviceIdOk, paramPidOk,       paramPidSignOk,
                      paramModeOk, paramLogLevelOk, paramGrpNameErr1, paramGrpNumErr1};
    int32_t argc12 = 8;
...
{
    AicpuDrvManager& drvMgr = AicpuDrvManager::GetInstance();
    MOCKER(drvQueryProcessHostPid).stubs().will(invoke(drvQueryProcessHostPidFake4));
    drvMgr.hostPid_ = 2;
    int32_t bindPidRet = drvMgr.CheckBindHostPid();
    EXPECT_EQ(bindPidRet, AICPU_SCHEDULE_ERROR_DRV_ERR);
}
...
TEST_F(AICPUScheduleTEST, ModelLoad_failed4)
{
    ...
    MOCKER_CPP(&AicpuModel::LoadQueueInfo).stubs().will(returnValue(0));

    MOCKER_CPP(&AicpuModel::AttachReportStatusQueue).stubs().will(returnValue(-1));

    MOCKER_CPP(&AicpuModel::ModelDestroy).stubs().will(returnValue(0));
...
int StubDclose(void* __handle) { return 0; }
