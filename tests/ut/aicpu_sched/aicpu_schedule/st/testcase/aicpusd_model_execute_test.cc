...
TEST_F(AICPUModelExecuteTEST, AICPUModelStop_fail1) { EXPECT_EQ(AICPUModelStop(nullptr), AICPU_SCHEDULE_FAIL); }
...
TEST_F(AICPUModelExecuteTEST, ModelExecute_failed2)
{
    ...
    MOCKER_CPP(&AicpuModel::CheckOperateAndUpdateStatus).stubs().will(returnValue(0));
    MOCKER_CPP(&AicpuModel::ResetModelForExecute).stubs().will(returnValue(-1));
