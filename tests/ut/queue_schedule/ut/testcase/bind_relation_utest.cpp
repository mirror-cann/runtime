...
TEST_F(BindRelationUTest, BindRelation_TEST_NUMA_SUCCESS)
{
    ...
    auto entity = dgw::EntityManager::Instance(0U).GetEntityById(
        bqs::LOCAL_Q, 0, dgw::EntityType::ENTITY_QUEUE, 1, dgw::EntityDirection::DIRECTION_SEND);
    ASSERT_TRUE(entity != nullptr);
    EXPECT_EQ(entity->GetResIndex(), 0U);
    EXPECT_EQ(entity->GetDeviceId(), 0U);
    entity = dgw::EntityManager::Instance(0U).GetEntityById(
        bqs::LOCAL_Q, 0, dgw::EntityType::ENTITY_QUEUE, 12, dgw::EntityDirection::DIRECTION_RECV);
    ASSERT_TRUE(entity != nullptr);
    EXPECT_EQ(entity->GetResIndex(), 0U);
    EXPECT_EQ(entity->GetDeviceId(), 0U);
    entity = dgw::EntityManager::Instance(0U).GetEntityById(
        bqs::LOCAL_Q, 1, dgw::EntityType::ENTITY_QUEUE, 13, dgw::EntityDirection::DIRECTION_RECV);
    ASSERT_TRUE(entity != nullptr);
    EXPECT_EQ(entity->GetResIndex(), 0U);
    EXPECT_EQ(entity->GetDeviceId(), 1U);
    entity = dgw::EntityManager::Instance(0U).GetEntityById(
        bqs::LOCAL_Q, 1, dgw::EntityType::ENTITY_QUEUE, 2, dgw::EntityDirection::DIRECTION_SEND);
    ASSERT_TRUE(entity != nullptr);
    EXPECT_EQ(entity->GetResIndex(), 0U);
    EXPECT_EQ(entity->GetDeviceId(), 1U);
    entity = dgw::EntityManager::Instance(1U).GetEntityById(
        bqs::LOCAL_Q, 1, dgw::EntityType::ENTITY_QUEUE, 3, dgw::EntityDirection::DIRECTION_SEND);
    ASSERT_TRUE(entity != nullptr);
    EXPECT_EQ(entity->GetResIndex(), 1U);
    EXPECT_EQ(entity->GetDeviceId(), 1U);
    entity = dgw::EntityManager::Instance(1U).GetEntityById(
        bqs::LOCAL_Q, 1, dgw::EntityType::ENTITY_QUEUE, 14, dgw::EntityDirection::DIRECTION_RECV);
    ASSERT_TRUE(entity != nullptr);
    EXPECT_EQ(entity->GetResIndex(), 1U);
    EXPECT_EQ(entity->GetDeviceId(), 1U);
