...
{
    char buf[256] = {0};
    aclError ret = aclplatformGetInstructionInfo(ACL_PLATFORM_CORE_TYPE_AI_CORE, "Intrinsic_vadd", buf, sizeof(buf));
    EXPECT_EQ(ret, ACL_SUCCESS);
...
{
    char buf[256] = {0};
    aclError ret = aclplatformGetInstructionInfo(ACL_PLATFORM_CORE_TYPE_AI_CORE, "Intrinsic_vmul", buf, sizeof(buf));
    EXPECT_EQ(ret, ACL_SUCCESS);
    EXPECT_STREQ(buf, "float16,float32,int32,int16");
}
...
{
    char buf[256] = {0};
    aclError ret = aclplatformGetInstructionInfo(ACL_PLATFORM_CORE_TYPE_AI_CORE, "Intrinsic_vrec", buf, sizeof(buf));
    EXPECT_EQ(ret, ACL_SUCCESS);
...
TEST_F(PlatformManagerAclUTest, acl_platform_GetInstructionInfo_VECTOR_CORE_vadd)
{
    char buf[256] = {0};
    aclError ret =
        aclplatformGetInstructionInfo(ACL_PLATFORM_CORE_TYPE_VECTOR_CORE, "Intrinsic_vadd", buf, sizeof(buf));
    EXPECT_EQ(ret, ACL_SUCCESS);
    EXPECT_STREQ(buf, "float16,float32,int32,int16");
}
...
TEST_F(PlatformManagerAclUTest, acl_platform_GetInstructionInfo_VECTOR_CORE_vmul)
{
    char buf[256] = {0};
    aclError ret =
        aclplatformGetInstructionInfo(ACL_PLATFORM_CORE_TYPE_VECTOR_CORE, "Intrinsic_vmul", buf, sizeof(buf));
    EXPECT_EQ(ret, ACL_SUCCESS);
    EXPECT_STREQ(buf, "float16,float32,int32,int16");
}
...
{
    char buf[256] = {0};
    aclError ret = aclplatformGetInstructionInfo(ACL_PLATFORM_CORE_TYPE_AI_CORE, nullptr, buf, sizeof(buf));
    EXPECT_EQ(ret, ACL_ERROR_INVALID_PARAM);
}

TEST_F(PlatformManagerAclUTest, acl_platform_GetInstructionInfo_null_value)
{
    aclError ret = aclplatformGetInstructionInfo(ACL_PLATFORM_CORE_TYPE_AI_CORE, "Intrinsic_vadd", nullptr, 256);
    EXPECT_EQ(ret, ACL_ERROR_INVALID_PARAM);
}
...
{
    char buf[256] = {0};
    aclError ret = aclplatformGetInstructionInfo(ACL_PLATFORM_CORE_TYPE_AI_CORE, "Intrinsic_vadd", buf, 0U);
    EXPECT_EQ(ret, ACL_ERROR_INVALID_PARAM);
}
...
TEST_F(PlatformManagerAclUTest, acl_platform_GetInstructionInfo_invalid_core_type)
{
    char buf[256] = {0};
    aclError ret =
        aclplatformGetInstructionInfo(static_cast<aclplatformCoreType>(99), "Intrinsic_vadd", buf, sizeof(buf));
    EXPECT_EQ(ret, ACL_ERROR_INVALID_PARAM);
}
...
TEST_F(PlatformManagerAclUTest, acl_platform_GetInstructionInfo_not_found)
{
    char buf[256] = {0};
    aclError ret =
        aclplatformGetInstructionInfo(ACL_PLATFORM_CORE_TYPE_AI_CORE, "Intrinsic_NotExist", buf, sizeof(buf));
    EXPECT_EQ(ret, ACL_ERROR_INVALID_PARAM);
}
