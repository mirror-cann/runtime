...
if ! ./build/main | tee "${file_path}"; then
    echo "[FAILURE] Context query sample failed."
    exit 1
fi

if grep -q "\\[SUCCESS\\] Context query sample completed successfully" "${file_path}"; then
    echo "[SUCCESS] Context query sample executed successfully."
else
    echo "[FAILURE] Context query sample did not print the expected success marker."
    exit 1
fi
